...
public class PhoneARCamera : MonoBehaviour
{
    ...
    public void OnScanExit() // does same thing as OnRefres() but specifically is for clearing all objects before switching to Journal Screen
    {
            ...
            foreach (var outline2 in this.boxSavedOutlines)
            {
                ...
                if (IsSameObject(outline1, outline2))
                {
                    ...
                    if (outline1.Confidence > outline2.Confidence + 0.05F) //& outline2.Confidence < 0.5F)
                    {
                        ...
                        anchorName.text =  $"{outline1.Label}";
                        Debug.Log("this is anchor name: " + anchorName.text);
    ...
    public string AnchorName(BoundingBox outline){
        string nameOfAnchor;
        
        var outline1 = $"{outline.Label}";
        nameOfAnchor = outline1.ToString();
        Debug.Log("name of the anchor: " + nameOfAnchor);
        return nameOfAnchor;
    }
    ...
    static void Main(){
        ReadUSDAFile readUSDAFile = new ReadUSDAFile();
        int i = 1;
        string[] entries = {"1029130213", "^", "rice", "^", "0.234"};
        var value = readUSDAFile.ReturnEntryName(i, entries);


        Debug.Log("value: " + value);
        BoundingBox box = new BoundingBox();
        box.Label = "rice";
        string outline = box.Label;

        PhoneARCamera phoneAR = new PhoneARCamera();
        var phone = phoneAR.AnchorName(box);
